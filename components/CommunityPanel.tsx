...
export const CommunityPanel: React.FC = () => {
  ...
  const submitUpdatePost = async (e: React.FormEvent) => {
      e.preventDefault();
      console.log(">>> [DEBUG] submitUpdatePost START");
      ...
      if (!isAdminUser || !userProfile) {
          console.error(">>> [DEBUG] Blocked: Not Admin or Profile Missing");
          return;
      }
      if (!updateTitle.trim() || !updateContent.trim()) {
          console.warn(">>> [DEBUG] Blocked: Title or Content Empty");
      ...
      try {
          console.log(">>> [DEBUG] Invoking communityService.createPost...");
          // Strict DB Mode
          const success = await communityService.createPost({
              title: updateTitle,
          ...
          console.log(`>>> [DEBUG] communityService response: ${success}`);

          if (success) {
            console.log(">>> [DEBUG] Success flow: Resetting form and fetching posts");
            ...
            console.error(">>> [DEBUG] Failure flow: Setting error message");
            setUpdateError("공지 등록 실패 (DB 연결/권한 확인)");
          }
      } catch (err) {
          console.error(">>> [DEBUG] Exception in submitUpdatePost:", err);
          setUpdateError("알 수 없는 오류 발생");
      } finally {
          setIsSubmittingUpdate(false);
          console.log(">>> [DEBUG] submitUpdatePost END");
      }
  };
                ...
                <div className="bg-white w-full max-w-lg rounded-3xl p-0 shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <div className="p-5 bg-slate-50 border-t border-slate-200 flex gap-3 flex-shrink-0">
                        ...
                        <button 
                            ...
                            onClick={() => console.log(">>> [DEBUG] 'Upload' Button Clicked. Title:", updateTitle)}
